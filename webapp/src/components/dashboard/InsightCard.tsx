  ...
  return (
    ...
    >
      ...
      <div className="flex items-start gap-3 mb-3 relative z-10">
        <div
          className={cn(
            "w-8 h-8 md:w-10 md:h-10 rounded-lg md:rounded-xl flex items-center justify-center shrink-0 transition-all duration-300",
            "group-hover:scale-110 group-hover:shadow-lg",
            insight.type === "celebration" && "bg-primary/20",
            insight.type === "pattern" && "bg-accent/20",
            insight.type === "suggestion" && "bg-warning/20",
        ...
        >
          <Icon
            className={cn(
              ...
              insight.type === "tip" && "text-primary/60"
            )}
          />
        </div>
        <div className="min-w-0">
          <p className="text-[8px] md:text-[10px] font-black uppercase tracking-[0.2em] md:tracking-[0.3em] text-muted-foreground/40 mb-1">
          ...
          <h4 className="font-black text-white text-xs md:text-sm uppercase tracking-tight truncate group-hover:text-primary transition-colors">{insight.title}</h4>
        </div>
      </div>

      {/* Message */}
      <p className="text-[10px] md:text-xs text-muted-foreground/80 leading-relaxed font-medium mb-4 relative z-10">
        {insight.message}
      </p>
      ...
      {insight.action ? (
        <Link
          ...
          className="inline-flex items-center text-[10px] md:text-xs font-black uppercase tracking-[0.2em] text-primary hover:text-primary/80 transition-all group/link relative z-10 mb-2"
        >
          {insight.action.label}
          <ArrowRight className="w-3.5 h-3.5 ml-2 transition-transform group-hover/link:translate-x-2" />
        </Link>
      ) : null}
